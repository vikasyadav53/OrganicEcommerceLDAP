...
public class LdapOperationsClient {
	...
	public void addUser(UserDetails user) throws Exception{
		...
		ldapTemplate.bind("uid="+user.getContactDetails().getEmail()+",ou=customer,dc=organicecommerce,dc=com",null, userAttr);
	...
	public void modifyUserDetails(UserDetails user) {
		...
		if(user.getUserId() != null && !StringUtils.isEmpty(user.getUserId())) {
			item = new ModificationItem(DirContext.REPLACE_ATTRIBUTE, new BasicAttribute("customerId", user.getUserId()));
			list.add(item);
		}
		if(user.getUserName() != null && !StringUtils.isEmpty(user.getUserName())) {
			item = new ModificationItem(DirContext.REPLACE_ATTRIBUTE, new BasicAttribute("customerName", user.getUserName()));
			list.add(item);
		}
		if(user.getUserPwd() != null && !StringUtils.isEmpty(user.getUserPwd())) {
			item = new ModificationItem(DirContext.REPLACE_ATTRIBUTE, new BasicAttribute("customerPwd", user.getUserPwd()));
			list.add(item);
		}
		if(user.getGender() != null && !StringUtils.isEmpty(user.getGender())) {
			item = new ModificationItem(DirContext.REPLACE_ATTRIBUTE, new BasicAttribute("customerGender", user.getGender()));
			list.add(item);
		}
		...
		if(user.getCreaterName() != null && !StringUtils.isEmpty(user.getCreaterName())) {
			item = new ModificationItem(DirContext.REPLACE_ATTRIBUTE, new BasicAttribute("createrName", user.getCreaterName()));
			list.add(item);
		}
		if(user.getLastModiferName() != null && !StringUtils.isEmpty(user.getLastModiferName())) {
			item = new ModificationItem(DirContext.REPLACE_ATTRIBUTE, new BasicAttribute("modifyName", user.getLastModiferName()));
			list.add(item);
		}
		if(user.getContactDetails().getEmail() != null && !StringUtils.isEmpty(user.getContactDetails().getEmail())) {
			item = new ModificationItem(DirContext.REPLACE_ATTRIBUTE, new BasicAttribute("custEmailAddress", user.getContactDetails().getEmail()));
			list.add(item);
		}
		if(user.getContactDetails().getPhone() != null && !StringUtils.isEmpty(user.getContactDetails().getPhone())) {
			item = new ModificationItem(DirContext.REPLACE_ATTRIBUTE, new BasicAttribute("phone", user.getContactDetails().getPhone()));
			list.add(item);
		}
		item = new ModificationItem(DirContext.REPLACE_ATTRIBUTE, new BasicAttribute("modifyTime",System.currentTimeMillis()));
		list.add(item);
		
		ldapTemplate.modifyAttributes("uid="+user.getContactDetails().getEmail()+"," +
				"ou=customer,dc=organicecommerce,dc=com", (ModificationItem[])list.toArray());
